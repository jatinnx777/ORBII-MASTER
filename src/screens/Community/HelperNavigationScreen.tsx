...
export function HelperNavigationScreen() {
  ...
  const { name, phone, lat, lng, photoUri } = route.params;
  ...
  // Live-track the helper's position.
  useEffect(() => {
    let watcher: LocationWatcher | null = null;
    watchLocation((point) => setMe(point), {
  ...
  const arrived = (straightMeters ?? 9999) < 30;
      ...
      <SafeAreaView edges={['bottom']} style={styles.bottomSafe} pointerEvents="box-none">
        <View style={styles.card}>
          ...
          <View style={styles.metricsRow}>
            ...
            <View style={styles.metricDivider} />
            <View style={styles.metric}>
              <View style={[styles.liveDot, arrived && { backgroundColor: colors.sage }]} />
              <Text style={styles.metricLabel}>{arrived ? 'Arrived' : 'Live'}</Text>
            </View>
          </View>
          ...
          >
            <Ionicons
              ...
              size={18}
              color={arrived ? colors.textInverse : colors.textPrimary}
            />
            <Text style={[styles.arriveText, arrived && { color: colors.textInverse }]}>
              {arrived ? "I've arrived" : 'Mark arrived'}
            </Text>
          </Pressable>
        </View>
      </SafeAreaView>
