...
import { OSMMapView, type OSMMarker } from '@/components/common';
...
function avatarHtml(name: string | null, color: string): string {
  const initial = (name || '?').slice(0, 1).toUpperCase();
  return `<div style="width:38px;height:38px;border-radius:50%;background:${color};border:3px solid #fff;box-shadow:0 3px 10px rgba(0,0,0,0.35);display:flex;align-items:center;justify-content:center;color:#fff;font-family:sans-serif;font-weight:700;font-size:15px">${initial}</div>`;
}
...
export function CircleMapScreen() {
  ...
  const markers: OSMMarker[] = members.map((m) => ({
    ...
    html: avatarHtml(m.name, colorFor(m.userId)),
  }));

  return (
    <View style={styles.root}>
      {center ? (
        <OSMMapView style={StyleSheet.absoluteFill} center={center} zoom={13} markers={markers} fitAll={markers.length > 0} />
      ) : (
              ...
              {members.map((m) => (
                <View key={m.userId} style={styles.memberRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.memberMeta}>Updated {ago(m.updatedAt)}</Text>
                  </View>
                  {m.battery != null ? (
...
const styles = StyleSheet.create({
  ...
  memberMeta: { fontFamily: fontFamilies.interMedium, fontSize: 12, color: colors.textSecondary, marginTop: 1 },
