...
class CirclesService {
  ...
  /// Create a shareable invite (no specific target) and return it so the UI
  /// can build a `https://orbii.app/join/<token>` link for WhatsApp/SMS. The
  /// `token` is generated by the DB default.
  static Future<CircleInvite> createInviteLink(String circleId) async {
    final uid = _uid;
    if (uid == null) throw Exception('Sign in to create an invite link.');
    final row = await _c
        .from('circle_invites')
        .insert({'circle_id': circleId, 'inviter_id': uid})
        .select()
        .single();
    return CircleInvite.fromRow(Map<String, dynamic>.from(row));
  }

  /// Accept an invite from its share token (`orbii://join/<token>` or
  /// `https://orbii.app/join/<token>`). Port of RN `acceptInviteByToken`.
  static Future<String?> acceptInviteByToken(String token) async {
    final uid = _uid;
    if (uid == null) throw Exception('Sign in to accept this invite.');
    final invite = await _c
        .from('circle_invites')
        .select('id, circle_id, status, expires_at')
        .eq('token', token)
        .maybeSingle();
    if (invite == null) throw Exception('This invite link is no longer valid.');
    final circleId = invite['circle_id'] as String;
    final status = invite['status'] as String?;
    if (status == 'accepted') return circleId;
    if (status != 'pending') throw Exception('This invite is no longer active.');
    try {
      await _c
          .from('circle_members')
          .insert({'circle_id': circleId, 'user_id': uid, 'role': 'member'});
    } catch (e) {
